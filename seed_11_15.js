...
async function seed() {
  try {
	...
	// --- Existing Categories above ---

	// New Pizza Example
	const pizzas = await createNode("Pizzas");
	const italianPizzas = await createNode("Italian Pizzas", pizzas);
	await createNode("Margherita Pizza", italianPizzas, "item", {
	  vendors: [
		{ name: "Domino's", url: "https://www.dominos.com/menu/pizza/margherita" },
		{ name: "Pizza Hut", url: "https://www.pizzahut.com/menu/margherita" },
	  ],
	});
